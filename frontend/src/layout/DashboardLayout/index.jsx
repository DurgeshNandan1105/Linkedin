...
export default function DashboardLayout({children}) {
  ...
  useEffect(() => {
      if (authState.profileFetched && authState.isError && !authState.user) {
          localStorage.removeItem("token");
          router.push("/login");
      }
  }, [authState.profileFetched, authState.isError, authState.user, router]);
...
{authState.all_profiles_fetched && authState.all_users.map((profile) => {
  return (
    <div key={profile._id} className={styles.extraContainer_profile}>
      <p>{profile.userId?.name || profile.name}</p>
      </div>
  )
